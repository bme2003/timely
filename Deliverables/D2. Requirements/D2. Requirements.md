...
trigger: selects find study group
pre-conditions: must be logged in
post-conditions: creates study group.

Success Scenario:
1.User goes to make study group
2.System requests other user profiles to be added to the group
3.User adds profiles
4.System sends invites out to those profiles for the study group
5.Other users accept invites
6.System adds users to the group

Alternate scenarios:
3a. User inputs invalid user
-The system asks for another user and tells the user that profile was invalid.

5a. Users reject the invite
-the system doesn't add them to the group
